  ...
  const connectProducer = async () => {
    ...
    while (true) {
      try {
        ...
        retries++;
        console.error(`Failed to connect Kafka producer (attempt ${retries}):`, error);
        await new Promise((resolve) => setTimeout(resolve, 5000));
      }
    }
  };

  // Start connection in background so it doesn't block WS server start entirely
  connectProducer();
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', async (rawMessage) => {
      try {
        ...
        if (!data || !toUserId || (!messageBody && (!data.attachments || data.attachments.length === 0)) || !conversationId) {
