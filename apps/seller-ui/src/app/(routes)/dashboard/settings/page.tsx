...
const SettingsPage = () => {
  ...
  const { data: settingsData, isLoading: settingsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance.get('/seller/api/settings');
      return res.data;
    },
  });
  ...
  useEffect(() => {
    if (settingsData?.settings) {
      reset({
        lowStockThreshold: settingsData.settings.lowStockThreshold?.toString() || '10',
        notificationPreference: settingsData.settings.notificationPreference || 'all',
        termsURL: '',
        privacyURL: '',
        refundURL: '',
      });
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await axiosInstance.put('/seller/api/settings', {
        lowStockThreshold: parseInt(data.lowStockThreshold),
        notificationPreference: data.notificationPreference,
        emailNotifications: true, // Always true for now since user is setting preferences
      });
      return res.data;
    },
    onSuccess: () => {
            ...
            {expandedSection === 'lowStock' && (
              ...
              >
                ...
                <button
                  ...
                  className="mt-4 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed text-white px-4 py-2 rounded-md flex items-center gap-2"
                >
                  <Save size={18} />
