  ...
  const signUpMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api/seller-registertaion`,
        data
      );
          ...
          <>
            {!showOtp ? (
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <input
                  type="tel"
                  placeholder="977981234****"
                  className="w-full p-2 boder border-gray-300 outline-0 rounded-[4px] mb-1"
                  {...register('phone_number', {
                    required: 'Phone number is required',
                    pattern: {
                      value: /^(?:\+977|977)?(98\d{8}|97\d{8})$/,
              ...
              <div>
                ...
                >
                  {verifyOtpMutation.isPending ? 'Verifiying...' : 'Verify OTP'}
                </button>
