...
const updateProfilePicture = async ({
  ...
  const response = await axiosInstance.put('/seller/api/update-image', {
    editType,
    imageUrl,
    fileId, // Send fileId for avatar (needed to create images record)
  });
...
const ImageEditModal = ({ editType, onClose }: ImageEditModalProps) => {
  ...
  const [fileId, setFileId] = useState<string | null>(null); // Store ImageKit file_id
  ...
  const uploadToImageKit = async (file: File) => {
    ...
    try {
      ...
      if (response.data.url) {
        ...
        setFileId(response.data.file_id); // Store file_id from response
      }
    } catch (error) {
  ...
  const handleUpdate = () => {
    if (!imageUrl) return;
    updatePicture({ editType, imageUrl, fileId: fileId || undefined });
  };
