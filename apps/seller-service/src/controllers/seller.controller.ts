...
export const updateProfilePictures = async (
  ...
  try {
    ...
    if (editType === 'cover') {
      // coverBanner is a simple string field - direct update
      await prisma.shops.update({
        where: { id: shop.id },
        data: { coverBanner: imageUrl },
      });
    } else {
      // avatar is an images[] relation - need to create images record
      if (!fileId) {
        return next(
          new ValidationError('file_id is required for avatar updates')
        );
      }

      // Delete existing avatar images for this shop
...
// Save seller settings (preferences)
export const saveSellerSettings = async (
  req: any,
  res: Response,
  next: NextFunction
) => {
  try {
    const sellerId = req.seller?.id;
    const { lowStockThreshold, notificationPreference, emailNotifications } =
      req.body;
    ...
    const validPreferences = ['email', 'web', 'app', 'all'];
    if (
      ...
      return next(
        new ValidationError(
          'Invalid notification preference. Must be one of: email, web, app, all'
        )
      );
    }
    ...
    if (lowStockThreshold !== undefined && lowStockThreshold < 0) {
      return next(
        new ValidationError('Low stock threshold must be a positive number')
      );
    }
