...
const ProductCard = ({
  ...
  return (
    <div className="w-full min-h-[350px] h-max bg-white rounded-lg relative">
      ...
      <Link href={`/product/${product?.slug}`}>
        <img
          src={
            ...
            'https://images.unsplash.com/photo-1635405074683-96d6921a2a68?w=500'
          }
          alt={product?.title}
          width={300}
          height={300}
      ...
      {isEvent && timeLeft && (
        <div className="mt-2">
          <span className="inline-block text-xs bg-orange-100 te"></span>
        </div>
      )}
      <div className="absolute z-10 flex flex-col gap-3 right-3 top-10">
        <div className="bg-white rounded-full p-[6px]  shadow-md">
          <Heart
            ...
            onClick={() =>
              isWishlisted
                ? removeFromWishlist(product.id, user, location, deviceInfo) // Changed: removeFromWishList -> removeFromWishlist
                : addToWishlist(
                    { ...product, quantity: 1 },
                    user,
                    location,
