...
const SalesChart = dynamic(
  () => import('../../shared/components/charts/sale-chart').then((mod) => mod.SalesChart),
  { ssr: false, loading: () => <p className="text-white">Loading SalesChart...</p> }
);
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total Sellers', value: data?.totalSellers ?? 0 },
          { label: 'Total Orders', value: data?.totalOrders ?? 0 },
          { label: 'Total Revenue', value: `Rs.${data?.totalRevenue?.toFixed(0) ?? 0}` },
        ].map(({ label, value }) => (
          <div key={label} className="bg-slate-800 rounded-xl p-5">
            <p className="text-slate-400 text-sm">{label}</p>
            {isLoading ? (
              <div className="h-8 w-24 bg-slate-700 animate-pulse rounded mt-1" />
            ) : (
